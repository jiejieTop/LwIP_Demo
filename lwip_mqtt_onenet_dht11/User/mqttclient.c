...
//        b = 70;
        ...
        if(UPDATE_SUCCESS == res)
        {
            ...
            data_len = strlen((char *)p);
            uint8_t* q = pvPortMalloc(data_len+3);
          printf("%s",p);
//            data_len = 47;
////            uint8_t h,l;
////            h = (data_len>>8);
//            l = data_len;
            ...
            printf("data_len = %d\n",data_len);
//            *(uint8_t*)q = 0x03;
//            *(uint8_t*)(q+1) = ((data_len+4)&0xff00)>>8;
//            *(uint8_t*)(q+2) = (data_len+4)&0xff;
//            memcpy((uint8_t*)(q+3),p,data_len);
              *(uint8_t*)&q[0] = 0x03;
              *(uint8_t*)&q[1] = 0;
              *(uint8_t*)&q[2] = (data_len+3)&0xff;
              memcpy((uint8_t*)(&q[3]),p,data_len);
//            PRINT_INFO("1:%s,%d\r\n",__FILE__,__LINE__);
...
//            }
            //发布消息
//            ret = MQTTMsgPublish(MQTT_Socket,(char*)"$dp",QOS0,(uint8_t*)q);
//            if(ret >= 0)
//            {
//                printf("表明有数据交换\n");
//                //表明有数据交换
